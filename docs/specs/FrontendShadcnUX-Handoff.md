...
**Track 2 complete.** 6 phases, one commit-pair each, on `feat/frontend-track-2`. Net: +2 primitives (`hc-pagination`, `hc-dropdown-menu`), +3 icons (`chevron-left/right`, `more-horizontal`), all 4 data tables upgraded. Vitest **132 → 187** (+55). Full `yarn e2e` still to be run against a live stack before merge (per-page gates listed above).
