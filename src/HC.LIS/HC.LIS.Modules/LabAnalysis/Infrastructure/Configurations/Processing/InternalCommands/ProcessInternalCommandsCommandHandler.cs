...
namespace HC.LIS.Modules.LabAnalysis.Infrastructure.Configurations.Processing.InternalCommands;
...
) : ICommandHandler<ProcessInternalCommandsCommand>
{
    ...
    public async Task Handle(ProcessInternalCommandsCommand command, CancellationToken cancellationToken)
    {
        ...
        string sql = @$"SELECT
                    ...
                    FROM ""lab_analysis"".""InternalCommands"" AS ""Command""
        ...
        foreach (InternalCommandDto? internalCommand in internalCommandsList)
        {
            ...
            {
                const string updateOnErrorSql = @$"UPDATE ""lab_analysis"".""InternalCommands""
