...
namespace HC.LIS.Modules.UserAccess.Infrastructure.Configurations.Processing.InternalCommands;
...
) : ICommandHandler<ProcessInternalCommandsCommand>
{
    ...
    public async Task Handle(ProcessInternalCommandsCommand command, CancellationToken cancellationToken)
    {
        ...
        string sql = @$"SELECT
                    ...
                    FROM ""user_access"".""InternalCommands"" AS ""Command""
        ...
        foreach (InternalCommandDto? internalCommand in internalCommandsList)
        {
            ...
            {
                const string updateOnErrorSql = @$"UPDATE ""user_access"".""InternalCommands""
