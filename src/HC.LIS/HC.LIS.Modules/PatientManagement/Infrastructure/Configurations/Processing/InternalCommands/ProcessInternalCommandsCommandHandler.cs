...
namespace HC.LIS.Modules.PatientManagement.Infrastructure.Configurations.Processing.InternalCommands;
...
) : ICommandHandler<ProcessInternalCommandsCommand>
{
    ...
    public async Task Handle(ProcessInternalCommandsCommand command, CancellationToken cancellationToken)
    {
        ...
        string sql = @$"SELECT
                    ...
                    FROM ""patient_management"".""InternalCommands"" AS ""Command""
        ...
        foreach (InternalCommandDto? internalCommand in internalCommandsList)
        {
            ...
            {
                const string updateOnErrorSql = @$"UPDATE ""patient_management"".""InternalCommands""
