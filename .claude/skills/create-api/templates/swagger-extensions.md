# Template: SwaggerExtensions.cs

**Output path:** `src/HC.LIS/HC.LIS.API/Configuration/Extensions/SwaggerExtensions.cs`

Swashbuckle 7.x supports Minimal APIs via `AddSwaggerGen` + `UseSwagger` / `UseSwaggerUI`.
...
**Note:** `AddEndpointsApiExplorer()` is required for Minimal APIs — it registers the metadata
that Swashbuckle reads from `.WithName()`, `.WithSummary()`, `.Produces<T>()`, etc.
